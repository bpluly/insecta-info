...
def updateOccurrence(dbConn, dbcursor, fieldList, row):
    ...
    updateStringbase = sql.SQL("UPDATE NHM_Occurrence SET ({})=(%s) WHERE id = {}").format(
        sql.SQL(",").join(map(sql.Identifier, fieldList)),sql.Placeholder(row[0]))

#    if verbose == True:
    logger.info("FieldList:"+str(len(fieldList)))
    logger.info(updateStringbase)
    logger.info("Row content:%s",row)
    ...
    if testing == False:
      try:
        ...
        logger.exception("UPDATE FAILED Type Error:")
        return False
      return True
    else:
      if verbose:
...
#       replace all empty fields with None
        ...
        if testing == False:
          if updateOccurrence(dbConn, dbCursor, allFields, row):
            try:
              dbConn.commit()
            except psycopg2.OperationalError as e:
              error(e)
